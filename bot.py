...
# ====== SIMPLE INTENT / GREETINGS ======
_GREETINGS = {
    "привет", "прив", "здравствуйте", "здравствуй", "добрый день", "добрый вечер",
    "доброе утро", "доброй ночи", "хай", "hi", "hello", "hey"
}
...
def is_greeting(text: str) -> bool:
    ...
    if t in _GREETINGS:
        return True
    for g in ("добрый день", "добрый вечер", "доброе утро"):
        if g in t:
            return True
    return False
...
async def handle_message(message: types.Message, state: FSMContext):
    # Приветствие — отвечаем нормально, не форсим запись
    if is_greeting(text):
        await message.answer(
            """Привет! 😊 Я Анна — администратор.
Чем помочь?
• Записать на массаж — напишите дату и время (например: «завтра 18:30»), или /book
• Узнать цены/акции
